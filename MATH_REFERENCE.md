...
| `Node.MorokField` | **[-1,1]** | `RecalculateFieldsFromGrid`, `PropagateWaves`, `UpdateMemories` | **отклонение** Distortion биома от его природы (0 = биом в норме). Абсолют — через `GetAmbientMorok()` |
| `Node.ZaryanaField` | **[-1,1]** | те же три | **отклонение** Stability биома от его природы |
...
```
GridAvgMorok(B)   = среднее (Cell.State.Meta.Distortion − дефолт биома)
                                                                  (ОТКЛОНЕНИЕ, [-1,1])
GridAvgZaryana(B) = среднее (Cell.State.Meta.Stability − дефолт биома)
                                                                  (ОТКЛОНЕНИЕ, [-1,1])
M ← Lerp(M, GridAvgMorok,   GridBlendFactor)      // :202
Z ← Lerp(Z, GridAvgZaryana, GridBlendFactor)      // :203
```
`GridBlendFactor = 0.3` — задан «за шаг» в ассете, но применяется как
`clamp(GridBlendFactor / 0.2 · dt, 0, 1)` (§6.3 закрыт 2026-09-07): при
боевом шаге 0.2с это ровно прежние 0.3, но больше не меняется молча при
смене `FixedTimeStep`.

✅ **Обе ветки в ОДНОЙ системе отсчёта** — знаковое отклонение от природы
биома (Морок доведён вторым заходом 2026-09-07). Ноль поля = «биом в своей
природе», затухание = возврат к ней. Главное правило при любой правке:
**читать и применять поле в одной системе отсчёта** — смешение как раз и
дало двойной счёт в первой, сломанной версии этой правки.

⚠ Абсолютный уровень («насколько тут грязно») больше НЕ равен полю: для
порогов легендарных сущностей, вклада места в варку и отладочной
визуализации есть `UBiomeGraphSubsystem::GetAmbientMorok()` = дефолт биома
+ отклонение. Читать поле напрямую там — значит сравнивать отклонение ~0 с
авторским порогом ~0.6, то есть молча никогда не срабатывать.
...
`GlobalInfluenceScale = 1.0`. Поток умножается на `dt / 0.2` (§6.3 закрыт
2026-09-07) — при боевом шаге поведение прежнее, данные ассета не трогались.
...
```
всё считается по ОТКЛОНЕНИЮ от дефолта биома, затем прибавляется дефолт:
...
**Равновесие ветки (при Push/Decay = 1):**
`D → дефолт_D + M`, `S → дефолт_S + Z`, `P → дефолт_P + 0.5·Z`.
Затухание полей в ноль (§3.4) означает возврат ВСЕХ трёх осей к дефолтам
своего биома — то, ради чего делался вариант «а» и его продолжение по Мороку.
...
```
Раньше это было **единственное** место, где дефолт биома попадал в
`TargetState` после инициализации, и якоря к природе биома в системе не
было вовсе. С 2026-09-07 якорем служит сама ambient-ветка (§3.3): её
равновесие — дефолт биома по всем трём осям.
...
```
✅ Комментарий на :2026 раньше называл сходимость «экспоненциальной» — это
было неверно (форма линейная), исправлено 2026-09-07. На корректность
`CatchUpActivatedChunks` не влияло (для линейного приближения догон одним
шагом `Step·N` как раз точен), но
формулировка в коде теперь тоже верная.
...
> ### ✅ 6.4 Мёртвая зона сторожа разреженности — 0.05 по любой оси
> `ApplyBiomeInfluences` пишет в `TargetState` только если изменение больше
> `KINDA_SMALL_NUMBER` (1e-4) — защита разреженного сохранения (§7.1
> `AUDIT_AND_REFACTORING_PLAN.md`). Шаг «дырявого ведра» равен
> `|отклонение|·Decay·dt`, поэтому движение **останавливается**, когда
> ```
> |отклонение| < KINDA_SMALL_NUMBER / (Decay·dt) = 1e-4 / (0.01·0.2) = 0.05
> ```
> **Замерено ровно так:** Purity, отпущенная с 0.30 при дефолте биома 0.70,
> поднимается до 0.65 и встаёт (тест
> `Herbalist.ApplyBiomeInfluences.PurityReturnsToItsOwnBiomeDefault`).
> То есть «возврат к дефолту биома» на практике означает «к дефолту ±0.05».
> Свойство общее для обеих веток и существовало всегда; при уменьшении
> `Decay` или `FixedTimeStep` мёртвая зона РАСТЁТ обратно пропорционально —
> это надо помнить при любой перенастройке темпов.

> ### ✅ 6.2 Purity/Stability тянутся к функции от Distortion — ИСПРАВЛЕНО 2026-09-07 (вариант «а»)
...
> ```
> Purity теряла 0.15 за 300 секунд и продолжала падать — прямое нарушение
> принципа «биом не портится без внешней причины» (Purity ↓ = порча).
>
> **Исправлено 2026-09-07 по выбору пользователя (вариант «а»):**
> `ZaryanaValue` больше не `1 − Distortion`, а **знаковое отклонение
> `Stability` от дефолта биома**; ветка Заряны в `ApplyBiomeInfluences`
> тоже переведена на отклонения (`S = дефолт_S + ведро`, `P = дефолт_P +
> 0.5·ведро`). Читается и применяется в ОДНОЙ системе отсчёта — двойного
> счёта, погубившего аналогичную правку по Distortion, здесь нет.
> Равновесие: `Z → 0` ⟹ обе оси возвращаются к дефолтам своего биома
> (±0.05 мёртвой зоны, §6.4). Проверено тестом на НАСТОЯЩЕЙ таблице биомов:
> Purity 0.30 → 0.65 при дефолте Тайги 0.70.
>
> Побочно: `PipelineV2.cpp:667` читал `ZaryanaField` как «чистоту места» для
> варки. Поскольку смысл поля сменился, там теперь явное `1 − MorokField` —
> это сохраняет прежнее поведение варки, а не меняет её баланс молча.
...
### 6.5 Проверено отдельно 2026-09-07 (то, что не попадало в прежние заходы)

- **Совместимость сохранений при смене смысла поля.** `MorokField`/
  `ZaryanaField` сериализуются (`Save->BiomeGraphNodes`), а `SaveVersion`
  защищал только от файлов НОВЕЕ текущей сборки. Смена семантики (абсолют →
  отклонение) сделала бы старый сейв Болота (0.70 абсолютных) миром с
  ambient-Мороком 1.0 — **молча, без ошибки в логе**. Введена версия 2 и
  миграция `MigrateBiomeGraphNodesV1ToV2` (Морок пересчитывается в
  отклонение, Заряна обнуляется — у старой величины `1−Distortion` нет
  осмысленного соответствия отклонению Stability). Тест:
  `Herbalist.Save.BiomeGraphV1NodesMigrateToDeviations`.
  **Правило на будущее:** смена СМЫСЛА сериализуемого поля требует версии
  и миграции ровно так же, как смена его типа.
- **Догон простаивавших чанков** (`CatchUpActivatedChunks` → один вызов
  `RegenerateCellParameters(Elapsed)` с большим `Elapsed`). Безопасен:
  `MoveToward` линеен и клампится в цель, поэтому N шагов по dt ≡ один шаг
  по N·dt. Комментарий в коде, объяснявший это «экспоненциальной формой
  сходимости», был неверен и исправлен. Контагион при большом `Elapsed`
  честно накапливает `Rate·Elapsed` (с клампом) — поведение до-существующее,
  не менялось.
- **Устойчивость «дырявого ведра» к большому шагу.** Явная схема Эйлера:
  при `Decay·dt > 1` пойдёт перерегулирование с колебаниями. Сейчас
  безопасно с огромным запасом (`0.01·0.2 = 0.002`), потому что
  `ApplyBiomeInfluences` вызывается ТОЛЬКО из шага графа с фиксированным
  `FixedTimeStep`. **Никогда не заводить ambient-ветку в путь догона
  чанков без перехода на аналитическую форму** — там `dt` измеряется
  сотнями секунд.
- **Вода и абсолютный уровень.** Отклонение считается от `GetDefaultWaterState`
  для водных клеток и от `GetDefaultState` для земли (верно), но
  `GetAmbientMorok` восстанавливает абсолют по СУХОПУТНОМУ дефолту биома.
  Это осознанное соглашение: `MorokField` — величина уровня БИОМА, где
  смешаны и вода, и суша, и единой «правильной» точки отсчёта у неё нет.
  Для биома, состоящего преимущественно из воды, абсолютная реконструкция
  будет смещена — помнить при чтении порогов легендарных сущностей.
...
### 7.1 Ловушки, на которых уже ошибались (сверяться перед выводом)

- **`Direction` НОРМАЛИЗУЕТСЯ.** `FBiomeDefaults::GetDefaultState` зовёт
  `Direction.NormalizeSum()`, а в таблице суммы порядка 2.0. Значит Nature
  Болота приезжает как **0.325**, а не как сырые 0.6 из строки. Пороги
  карточек по осям Body/Mind/Spirit/Nature сравниваются с НОРМАЛИЗОВАННЫМ
  значением. Ошибся ровно на этом 2026-09-07 при аудите карточек Низшего
  ранга — половина выводов по Nature-осям поменялась после пересчёта.
  Инвариант под тестом `BiomeDefaults.RealTableIsAvailableWithoutGameMode`.
- **Числа берутся из АССЕТА, не из `CSV_tabs/*.json`.** Документный экспорт
  разошёлся с ассетом у шести биомов из восьми (см. §8). По ассету идёт
  игра; json — только документация, и уже устаревшая.
- **`Meta` и `Direction` — разные системы отсчёта у воды.** Водный дефолт
  отличается от сухопутного только у Тундры; у остальных семи совпадает.
  Полагаться на «у воды всегда своё» нельзя.
...
| 6.2 | `Purity`/`Stability` тянулись к `f(Distortion)` | ✅ **исправлено 2026-09-07** (вариант «а»: Заряна = отклонение от дефолта биома) |
| 6.4 | Мёртвая зона сторожа разреженности ±0.05 по каждой оси | ✅ измерено и задокументировано, поведение осознанное |
| — | **Автотесты гоняются на НУЛЕВЫХ дефолтах биомов** (`DT_BiomeDefaults` грузит только GameMode, в тестовом мире его нет) | ✅ **закрыто 2026-09-07** — `FBiomeDefaults` грузит таблицу лениво сам; из 12 павших тестов ни один не оказался багом игры, все были устаревшими (см. CHANGELOG) |
| — | Документный экспорт `CSV_tabs/DT_BiomeDefaults.json` расходится с настоящим ассетом у 6 биомов из 8 | ⚠ **найдено 2026-09-07** — вынесено в ROADMAP, нужен выбор источника правды |
| — | Благие Легендарные (порог 0.20) недостижимы при природном Distortion биомов 0.28…0.38; Болото стоит РОВНО на пороге Болотного царя | ⚠ **найдено 2026-09-07** — вынесено в ROADMAP, вопрос замысла |
| — | «Занял — держит вечно» у Низшего ранга: `CanManifest` не даёт вытеснить равного | ⚠ **найдено 2026-09-07** — вырожденный случай (порог 0.00) закрыт, само правило не трогалось |
| 6.3 | `GridBlendFactor`, `MorokLeak` не масштабированы на dt | ✅ **исправлено 2026-09-07** — пересчёт на чтении через опорный шаг 0.2с, данные ассета не трогались |
| 4.3 | Комментарий про «экспоненциальную сходимость» противоречит линейному коду | ✅ исправлен 2026-09-07 |
| 6.5 | Совместимость сейвов при смене смысла поля | ✅ **найдено и закрыто 2026-09-07** — SaveVersion 2 + миграция |
