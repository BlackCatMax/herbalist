...
```
GridAvgMorok(B)   = среднее Cell.State.Meta.Distortion            (АБСОЛЮТ, [0,1])
...
⚠ **Две ветки живут в РАЗНЫХ системах отсчёта** (с 2026-09-07): Морок —
абсолютный уровень, Заряна — знаковое отклонение от дефолта биома. Это не
недосмотр: Заряну перевели на отклонение по выбору пользователя (вариант «а»,
§6.2), чтобы Purity/Stability возвращались к своей природе; Морок оставлен
как был, потому что его текущее поведение подтверждено живым PIE. Главное
правило при любой правке: **читать и применять поле в ОДНОЙ системе
отсчёта** — смешение как раз и дало двойной счёт 2026-09-07.
...
`D → M` (абсолют), `S → дефолт_S + Z`, `P → дефолт_P + 0.5·Z`.
Затухание `Z → 0` (§3.4) означает возврат Purity/Stability к дефолтам
своего биома — то, ради чего делался вариант «а».
