...
| — | **Автотесты гоняются на НУЛЕВЫХ дефолтах биомов** (`DT_BiomeDefaults` грузит только GameMode, в тестовом мире его нет). Включение реальной таблицы роняет 9 тестов: 6 про то, какое существо проявится, `Save.BiomeInfluencesWithZeroFieldsStaySparse` (при реальных дефолтах Distortion-ветка дёргает ВСЕ 400 клеток каждый шаг — риск для разреженного сохранения §7.1), `PlaySession.SaveAndReloadMidSession`, `SystemInteraction.AmbientSweepAcrossAllBiomes` | ⚠ **найдено 2026-09-07, не чинилось** — отдельная работа с дизайн-суждением по каждому тесту |
