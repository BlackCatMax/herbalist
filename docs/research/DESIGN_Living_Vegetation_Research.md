...
Открытых вопросов по плану нет. Этап 1а выполнен 2026-09-16 (`Core/Types/HerbalistCalendar.h`, GDD §15.4), 1б и 2 — тем же днём (`Core/Types/HerbalistTimeDisplay.h`, `-run=TimeDisplaySetup`, консоль `SetGameClock`/`SkipGameDays`; мост `UUltraDynamicSkyBridge`, проверка в PIE ❓); функции этапа 3 — 2026-09-17 (`-run=MaterialFunctionsSetup`), подключение в мастер-материалы вручную: в `M_Foliage_Master` и `M_landscape` ручная сборка троп пользователя; 3б — тем же днём (`CSV_tabs/ingredient_flower_open_phase.json`); следующий — 4, листопад (Niagara у игрока, подстилка на земле).
