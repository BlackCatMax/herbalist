...
Осталось не в C++:
- PCG-компоненты травы — на акторах `BP_BiomeVolume` пользователя, режим
  `GenerateOnLoad`: граф запекается в редакторе, где сетки нет, и узел
  пропускает точки без сезона. Сезон заработает при `Generate At Runtime`
  (с разбиением на ячейки и радиусом генерации) — правка Blueprint и уровня,
...
| 5 🟡 2026-09-17: `SeasonKey`/`SeasonMeshKey` и сезонное прореживание в `Sample Herbalist Cell` ✅, узел в `PCG_Grass` ✅; `Generate At Runtime` у `BP_BiomeVolume` ❓ и сезонные меши — не C++ | **PCG**: `SeasonKey` × `MeshKey`, стратегия перегенерации, чистка `PCG_Grass` | смена сезона без видимого хлопка у игрока |
...
Открытых вопросов по плану нет. Этап 1а выполнен 2026-09-16 (`Core/Types/HerbalistCalendar.h`, GDD §15.4), 1б и 2 — тем же днём (`Core/Types/HerbalistTimeDisplay.h`, `-run=TimeDisplaySetup`, консоль `SetGameClock`/`SkipGameDays`; мост `UUltraDynamicSkyBridge`, проверка в PIE ❓); функции этапа 3 — 2026-09-17 (`-run=MaterialFunctionsSetup`), подключение в мастер-материалы вручную: в `M_Foliage_Master` и `M_landscape` ручная сборка троп пользователя; 3б — тем же днём (`CSV_tabs/ingredient_flower_open_phase.json`); 4 — тем же днём, C++-часть (`ULeafFallSubsystem`, `LeafFall01`/`LeafLitter01`; систему Niagara и слой подстилки делает художник); 5 — тем же днём (сезон в `Sample Herbalist Cell`, узел в `PCG_Grass`; генерация в рантайме у `BP_BiomeVolume` ждёт решения); следующий — 5б, слоты ресурсов, и 6, проверка в PIE.
