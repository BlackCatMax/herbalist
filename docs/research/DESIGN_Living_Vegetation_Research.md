...
Полнолуния. Этап 4 (2026-09-17): `LeafFall01` — нормированная производная
`LeafDrop01` на спаде листвы (`4t(1−t)`, пик в середине, весеннее отрастание не
листопад), `LeafLitter01` — подстилка: до середины зимы = `LeafDrop01`, с середины до
конца зимы 1, с начала до середины весны линейно к 0.
...
| `LeafFall01` | scalar | темп листопада: 0 вне спада листвы, 1 на пике (этап 4) | производная `LeafDrop01` |
| `LeafLitter01` | scalar | подстилка: растёт до середины зимы, лежит до конца зимы, к середине весны сходит (этап 4) | из `SeasonUDW` |
...
- **Падающие листья** — Niagara у игрока (эмиттер на источнике генерации или
  камере), темп ∝ скорость роста `LeafDrop01` × ветер, цвет из сезона. ✅ Этап 4:
  `LeafFall01 × (0.4 + 0.6 × ветер)` — в штиль листья падают медленнее, но
  падают (`LeafFallWindShare`). Только в
...
**Этап 3б (2026-09-17): начальные `OpenPhase`.**
`tools/data_extraction/extract_flower_open_phase.py` → `CSV_tabs/ingredient_flower_open_phase.json`:
цветущих видов — 42 (трава и кустарники с цветением в тексте карточки), окно
раскрытия = окну сбора (без окна — день). Пометки на ручную разметку — у
19 видов; у каждой пометки в json лежит предложение карточки, которое её
дало. Поиск по словам: пометка — повод прочитать, а не готовое расхождение
(есть и «хранить в закрытой посуде», и легенды). Сбор нераскрытым прямо назван у
видов: Сон-трава, Ветреница. У видов с неприметным цветком или со сбором ягод и
листьев (ковыль, крапива, брусника, калина) окно сбора к раскрытию цветка
отношения не имеет — их тоже размечать руками. Когда у видов появятся меши и
инстансы, `OpenPhase` в инстансы — из этого json.
...
**Этап 5 (2026-09-17, решение пользователя — вариант А ниже).** `SeasonKey` —
не параметр графа, а атрибут точки: его пишет `Sample Herbalist Cell` на
момент генерации (технический сезон), вместе с `SeasonMeshKey` =
`MeshKey_SeasonKey` (`Healthy_Winter`). При генерации в рантайме новый сезон
получают клетки, которые генерируются после смены, — без хлопка у игрока;
перезапускать граф из кода не нужно. Узел же прореживает траву по доле сезона
(`SpringDensity`…`WinterDensity`, по умолчанию 1 / 1 / 0.8 / 0.4),
детерминированно по сиду точки и вложенно: зимний набор — подмножество
осеннего. Узел не кэшируется PCG (ключ кэша — только настройки и входные
точки, без сезона и состояния клеток; иначе вернувшаяся клетка получала бы
прошлый сезон). `-run=PcgGrassSeasonSetup` вставил узел в `PCG_Grass` между
`World Raycast` и `Attribute Noise`.

Трава в рантайме ✅ (2026-09-19, решение пользователя «переключай в
рантайм»): `-Builder=PcgGrassRuntimeBuilder` перевёл шаблон PCG-компонента в
`BP_BiomeVolume` и два экземпляра на `L_TestDev` на `GenerateAtRuntime` с
разбиением на ячейки и вычистил запечённое; `PCG_Grass` — иерархическая
генерация с сеткой 64 м (радиус 128 м: объём строится ячейками по мере подхода,
без неё — целиком за 512 м по сетке карты 256 м); кэш ландшафта у PCG World
Actor — `SerializeOnlyAtCook` (при `NeverSerialize` в PIE и в сборке данных
ландшафта нет, проекция выбросила бы всю траву). Нагрузка не проверена ❓:
шаг сэмплера сплайна 10 см (~100 точек на м²), ячейка 64×64 м — до ~400 тыс.
точек, проекция и `Sample Herbalist Cell` на CPU (узел — на игровом потоке):
возможен рывок при подходе; снижать `InteriorSampleSpacing` или сетку.
Точки, чья проекция мимо ландшафта (попадание луча в камень), выбрасываются.

Исключение покраски `Ground` ✅ (2026-09-19, вопрос пользователя «трава
растёт на исключённой покраске»): фильтр сравнивал `Ground` с `@Last` —
постоянный порог 0.8 был выключен, — а точки-вычитатели с `Surface Sampler`
(0.5 на м², квадрат 50×50 см, со сдвигом) покрывали ~12% площади. Теперь
точкам травы проецируется ландшафт, тот же фильтр — «`Ground` < 0.8»
(постоянный) — стоит перед `Sample Herbalist Cell`, старая ветка убрана.
Слои `L_TestDev`: `Grass_Cliff` и `Ground`.

Осталось не в C++:
- Мешей травы по сезонам (пожухлая, под снегом) и для испорченных клеток в
  проекте нет — спавнеры `PCG_Grass` пока на `PCGMeshSelectorWeighted` с одним
  `SM_Grass`; выбор по `SeasonMeshKey` (`PCGMeshSelectorByAttribute`) — когда
  появятся меши.
- Долг `PCG_Grass` из ROADMAP частично закрыт правкой пользователя (коммит
  `e55ff13`): спавнеров три, не четыре; фильтры плотности 0–0.2, 0.21–0.4,
  0.41–1 делят точки на ярусы размера (зазоры 0.2–0.21 и 0.4–0.41 при шуме
  плотности 0..1 теряли бы около 2% травы — не сверено в редакторе ❓, в ROADMAP);
  ссылки на `MI_Grass_Sedge_02` нет; сэмплер ландшафта — 0.5 точки
  на м². Один меш на всех ярусах остаётся.
...
**✅ Этап 5б выполнен 2026-09-19** (решения пользователя 2026-09-19):

- **Как слоты доходят до C++** — запекаются в редакторе: граф
  `PCG_ResourceSlots` заканчивается узлом `Write Herbalist Resource Slots`,
  тот пишет точки в ассет карты `/Game/Data/ResourceSlots/RS_<карта>`
  (`UHerbalistResourceSlots`, набор на актор-источник). PCG в игре для
  ресурсов не работает; менеджер сетки загружает ассет в `BeginPlay`.
- **Где** — только у воды: компонент с графом на `BP_WaterVolume`. Вода —
  выборка поверхности внутренности сплайна (плоскость `SM_PoolRound` плоская и
  в плоскости сплайна, Z −37.9, выборка по мешу не нужна); кромка — полоса до
  1.5 м за сплайном по каждой оси (до ~2.1 м по диагонали), на ландшафте;
  суша — кольцо до 9 м (одна клетка; до ~12.7 м по диагонали) вокруг.
  Без кольца суши клетка у берега со слотами только воды и кромки отдавала бы
  водным видам все ресурсы. Остальные клетки — прежний разброс.
- **Как C++ выбирает** — место ресурса i берёт i-й слот клетки в порядке,
  перемешанном потоком клетки (по кругу). Вид — по виду слота: вода и
  кромка — водный пул (`bGrowsOnWater`), суша — земной пул без водных видов.
  Водный пул пуст или водному нет свободного слота — земной вид (кроме
  слота воды в водной клетке); пристройка сада и посадка — как до слотов. Ресурс встаёт ровно в слот, без сдвига региона;
  слот не годится виду или занят — следующий по порядку; нет — земному виду
  прежний разброс, водному — ничего.
- **Детерминизм** — слот места зависит только от сида клетки и индекса
  места, не от состояния: спящая клетка, пробуждение и загрузка сейва видят
  один и тот же слот, места до числа слотов — разные слоты. Отросшая трава
  получает новый индекс места и встаёт в следующий слот, не обязательно на
  прежнее место. Перезапекание заменяет набор своего актора, наборы в ассете
  по ключу — порядок запекания не важен; набор удалённого актора остаётся в
  ассете, пока его не вычистят (ROADMAP).

Запекание: `TOOLS_REFERENCE.md`, `-run=PcgResourceSlotsSetup`.
...
| 3 🟡 2026-09-17: функции ✅, `OpenPhase` ✅ (3б, данные), подключение в мастера — вручную (`TOOLS_REFERENCE.md`) | **Функции материалов слоя сезона**: `MF_SeasonWeights`, `MF_SeasonColor`, `MF_LeafDrop` (порог маски), `MF_GrassSquash` (сжатие: тропа, зима, снег UDW), `MF_FlowerOpen`; подключение в `M_Foliage_Master`, `M_plants`, `M_landscape` вместе с уже готовыми `MF_*`; начальные `OpenPhase` с отчётом расхождений (скрипт данных, этап 3б) | `-verify` компилирует; в PIE при перемотке трава желтеет и ложится, листва редеет, цветы раскрываются в своё окно |
| 4 🟡 2026-09-17: C++ ✅ (`ULeafFallSubsystem`, `LeafFall01`/`LeafLitter01`), ассет Niagara и слой подстилки — художник (`TOOLS_REFERENCE.md`) | **Листопад**: истончение маски по `LeafDrop01`, Niagara у игрока, подстилка на ландшафте | за визуальную осень листья падают, к зиме ветки голые |
| 5 🟡 2026-09-19: `SeasonKey`/`SeasonMeshKey` и сезонное прореживание в `Sample Herbalist Cell` ✅, узел в `PCG_Grass` ✅, трава в рантайме ✅ (нагрузка ❓); сезонные меши — не C++ | **PCG**: `SeasonKey` × `MeshKey`, стратегия перегенерации, чистка `PCG_Grass` | смена сезона без видимого хлопка у игрока |
| 5б ✅ 2026-09-19 (PIE ❓): 213 слотов на `L_TestDev` | **Слоты ресурсов** (§4.1, решение пользователя): PCG ставит точки-слоты с тегом места, включая плоскость воды на сплайне биома воды; C++ выбирает вид и ставит ресурс в слот, без слотов — прежний разброс | водные растения стоят на воде; сбор, отрастание и сейв ресурсов в слотах работают как раньше; детерминизм по сиду клетки сохранён |
| 6 ✅ 2026-09-19: сценарии записаны (раздел «Живая растительность: сценарии»), прогон в PIE — за пользователем | **Проверка** в `docs/verification/pie/04_World_State.md` | сценарии перемотки на сутки, на границу сезона, на снегопад |
...
Открытых вопросов по плану нет. Этап 1а выполнен 2026-09-16 (`Core/Types/HerbalistCalendar.h`, GDD §15.4), 1б и 2 — тем же днём (`Core/Types/HerbalistTimeDisplay.h`, `-run=TimeDisplaySetup`, консоль `SetGameClock`/`SkipGameDays`; мост `UUltraDynamicSkyBridge`, проверка в PIE ❓); функции этапа 3 — 2026-09-17 (`-run=MaterialFunctionsSetup`), подключение в мастер-материалы вручную: в `M_Foliage_Master` и `M_landscape` ручная сборка троп пользователя; 3б — тем же днём (`CSV_tabs/ingredient_flower_open_phase.json`); 4 — тем же днём, C++-часть (`ULeafFallSubsystem`, `LeafFall01`/`LeafLitter01`; систему Niagara и слой подстилки делает художник); 5 — тем же днём (сезон в `Sample Herbalist Cell`, узел в `PCG_Grass`), трава в рантайме и исключение покраски `Ground` — 2026-09-19; 5б, слоты ресурсов, — тем же днём (§4.1); следующий — прогон этапа 6 в PIE.
