...
| # | Вопрос | Решение |
|---|---|---|
| 1 | Длина визуальной осени | **Уточнено:** технически сезонов четыре равных, как в UDS; игроку по лору их три — «Лето» = лето + осень, полгода, из них осень — вторая половина (§2.1). Это меняет механику, не только показ — последствия и вопросы в §2.3 |
...
| Осень по решению 2026-09-16 | ¼ года (§2.3) | ≈ 47 ч при годе 351 сутки, ≈ 62 ч при 468 |
| Год | 351 сутки | ≈ 187 ч |
...
**Решение пользователя: технически четыре равных сезона, по лору три.**

| Технический сезон | Доля года | Лоровый сезон (игроку) | `SeasonUDW` |
|---|---|---|---|
| Весна | ¼ | Весна | середина = 0 |
| Лето | ¼ | Лето (первая половина) | середина = 1 |
| Осень | ¼ | Лето (вторая половина) | середина = 2 |
| Зима | ¼ | Зима | середина = 3 |

Кривая показа: год `y ∈ [0,1)` от начала технической весны,
...
1. **Длина года.** (А) Сезон остаётся 117 суток → год 468 суток = 52 недели по
   9 суток, по 13 недель на сезон (строительный блок §15.4 сохраняется); лоровое
   Лето 234 суток ≈ 125 реальных часов, год ≈ 250 ч. (Б) Год остаётся 351 сутки
   → сезон 87.75 суток, «13 недель на сезон» ломается. Рекомендация — А.
2. **Месяцы.** «Лето — 6 месяцев»: месяцев в проекте нет. Вводить месяц как ⅓
   технического сезона (при А — 39 суток) для лора, журнала и даты UDS, или
   месяцы только в речи?
...
5. **Купальская ночь.** Сейчас 0.15…0.18 доли Лета. Купала в жизни — около
   летнего солнцеворота: окно вокруг середины технического лета? Рекомендация —
   да.
...
8. **UDS: сезон вручную или от даты.** Рекомендация — вручную (`Set Season` из
   часов проекта): одна запись; режим «от даты» потребовал бы держать
   синхронной ещё и дату UDS.
