...
| # | Вопрос | Решение |
|---|---|---|
| 1 | Длина визуальной осени | Лето проекта показывается как лето и обычная осень вместе; Весна и Зима — как есть. Механика остаётся трёхпольной |
...
| 5 | Функции сезонов UDW или свои | Разница разобрана в §2.2; рекомендация — свои веса сезона и производные, снег и мокрость — из UDW. Ждёт подтверждения |
| 6 | Окно раскрытия цветов | Рекомендация — не поле таблицы, а параметр инстанса материала вида с начальными значениями из `HarvestTimeWindow` и списком расхождений с прозой карточек (§3.3). Ждёт подтверждения |
...
4. **Три сезона проекта против четырёх у UDW.** Решение пользователя: Лето
   проекта показывается как лето и обычная осень вместе — визуальная осень
   занимает вторую часть Лета, механика остаётся трёхпольной (§2.1).
...
Соответствие по решению пользователя: **Лето проекта = лето + обычная осень**.
Год делится на три равных сезона, Лето показывается двумя визуальными
сезонами подряд. Доли внутри Лета и ширина переходов — числа показа, их
подбирать глазами (предложение ниже):

| Момент года (`GetSeasonProgress01`) | Сезон проекта | Визуальный сезон | `SeasonUDW` |
|---|---|---|---|
| Весна, середина | Весна | весна | 0 |
| Весна 0.9 → Лето 0.1 | граница | весна → лето | 0 → 1 |
| Лето 0.1 … 0.4 | Лето | лето | 1 |
| Лето 0.4 … 0.6 | Лето | лето → осень | 1 → 2 |
| Лето 0.6 … 0.9 | Лето | осень, листопад растёт | 2 |
| Лето 0.9 → Зима 0.1 | граница | осень → зима, листва опала | 2 → 3 |
| Зима | Зима | зима | 3 |
| Зима 0.9 → Весна 0.1 | граница | зима → весна | 3 → 4 (= 0) |

Визуальная осень занимает около 40% Лета — порядка 25 реальных часов вместо
12.5 у окна `IsLateSummer`. Механика сезонов не меняется. `IsLateSummer`
(Листовики, §16.2) лежит внутри визуальной осени — показ и бестиарий не
...
Рекомендация: **веса сезона и производные — свои**, **снег и мокрость — из
UDW** (там уже подключены). У материала ровно две коллекции: наша и UDW.
Проверить до этапа 3: какую коллекцию читают `DLWE_SnowCoverage` и
`Sample_UDW_Seasons` ❓.
...
| 1 | **Мост показа в C++**: параметры §2 в `MPC_WorldStateFields` (коммандлет), сглаживание, кривая сезонов §2.1 (Лето = лето + осень); консольная перемотка часов (`SetGameClock`/`SkipGameDays`) | автотесты кривых: веса в сумме 1, без скачков на границах сезонов и фаз суток, осень только внутри Лета; в PIE параметры меняются при перемотке |
...
Решения по первым семи вопросам — в разделе в начале документа. Осталось
подтвердить:

1. **§2.2** — веса сезона свои, снег и мокрость из UDW?
2. **§3.3** — окно раскрытия в инстансе материала, начальные значения из
   `HarvestTimeWindow` с отчётом расхождений?
3. **Доли визуальной осени внутри Лета** (§2.1: лето до 0.4, переход, осень с
   0.6) — подбирать глазами на этапе 3, или сразу задать своё соотношение?
