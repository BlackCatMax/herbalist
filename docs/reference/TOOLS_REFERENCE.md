...
| `-run=PcgGrassSeasonSetup` | Вставляет `Sample Herbalist Cell` в `/Game/PCG/PCG_Grass` между `World Raycast` и `Attribute Noise` (связь ищется по типам узлов): точки травы получают состояние клетки, `MeshKey`, `SeasonKey`, `SeasonMeshKey` и прореживаются по доле сезона. Спавнеры не трогает. Узел уже в графе — ничего не делает | 2026-09-17 |
