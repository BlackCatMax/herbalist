...
public:
    ...
    // Прежнее окно варки у котла под взглядом -- только для отладки: в игре
    // травы кладут в котёл рукой, пустой рукой котёл мешают (этап 3).
    UFUNCTION(Exec)
    void OpenCauldronWindow();

    // Прежнее окно переноса у хранилища под взглядом -- только для отладки:
    // в игре кладут из руки, берут из раскладки (этап 3).
    UFUNCTION(Exec)
    void OpenStorageWindow();

    // Сон (этап 6): проспать до следующего рассвета и сохраниться. Лавка у
    // дома и костёр в пути зовут это. false -- мира нет.
    bool SleepUntilDawn();
    // Вязанка для лагеря (карточка «Хворост», BrushwoodAppendCommandlet).
    static FName BrushwoodID() { return FName(TEXT("Хворост")); }
    ...
    // InvisibilityCapRadiusMeters), можно активировать снова после истечения
