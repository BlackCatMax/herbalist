...
//
// Капище — отдельное МЕСТО на карте (2026-09-02, пересмотр решения v1 по
// прямому запросу пользователя). Раньше капище не существовало само по себе:
// оно было там, где стоит котёл ("особая аура привязана к месту варки"), и
// регистрировалось из AAlchemyTableActor::BeginPlay. Теперь котёл капища не
