...
//      первыми). Purified/Catastrophe используют свою отдельную, более узкую
//      пару эпитетов вместо общей шестёрки — редкие исходы должны звучать
//      особенно, не просто "очень чистая обычная вода".
//
...
namespace
{
    ...
    const FPotionEpithet& GetValidEpithet(const FMeta& M)
    {
...
FText GeneratePotionName(EAlchemyOutcome Outcome, const FRealState& State, EGrammaticalCase Case)
{
    ...
    const FPotionEpithet& Epithet = GetValidEpithet(State.Meta);
