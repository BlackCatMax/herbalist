...
// трёхполье: осень -- часть Лета (AGridWorldManager::GetLoreSeason). Autumn --
// последним значением: прежние Spring/Summer/Winter в данных не сдвигаются.
UENUM(BlueprintType)
enum class ESeason : uint8
{
    ...
    Autumn    // Осень: сентябрь–ноябрь; по лору -- часть Лета
};
...
struct PROJECTHERBALIST_API FGridCell
{
    ...
    // Низший, вытесненный с клетки более редким (правило «редкое вытесняет
    // частое», 2026-09-19): пока вытеснитель держит клетку, вытесненный
    // сохраняет гистерезис удержания и возвращается по нему (ревью
    // 2026-09-20). Память на одну ступень (цепочка A->B->C помнит B). В сейв
    // не пишется, загрузка сбрасывает: вернётся по порогу входа -- лишняя
    // строгость на редком стыке, не потеря состояния.
    UPROPERTY(Transient)
    FName DisplacedEntityID = NAME_None;
    ...
    // падает, когда таймер срабатывает; неудачная попытка ставит новую
    // (2026-09-14), так что счётчик -- число мест, ждущих растения. Место
    // теряется только там, где клетке расти не положено: регион перестал её
    // заявлять или растит по PCG-графу (ранние выходы CompleteRegrowth). С того
    // же дня сохраняется в сейве (FSavedCellState::PendingRegrowthCount).
    UPROPERTY()
