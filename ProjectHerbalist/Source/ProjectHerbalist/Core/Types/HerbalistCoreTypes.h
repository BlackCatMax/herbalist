...
//
// Только два эффекта реализованы этим заходом (вертикальный срез, тот же
// принцип, что уже у сада/инструментов сбора): BrewBoost (Громовая стрела)
// и EntityConceal (Плакун-камень). Остальные фольклорные кандидаты,
// обсуждённые в разговоре, — MorokReduction (Куриный бог, снижение Морока
// восприятия — прецедент Perception::PerceiveValue/GlobalPerceptionClarity)
// и PathReveal (показ троп, опоры в компендиуме пока не найдено) —
// СОЗНАТЕЛЬНО не заведены как значения enum'а вовсе, не заглушки: добавлять
// значение без потребителя значило бы имитировать реализацию. См. ROADMAP.md.
UENUM(BlueprintType)
