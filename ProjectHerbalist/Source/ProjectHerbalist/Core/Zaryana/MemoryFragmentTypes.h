...
//
// Состояние на 2026-09-19: 13 фрагментов в DT_MemoryFragments (4 базовых +
// 3 исхода Буяна §18.1 + 5 биомных §17.7 + БРОД Болота, 23_Journey_Order
// §23.4) на 8 типов триггеров. Прежняя
...
// (60 карточек в трёх DataTable), а сами фрагменты выросли с 3 до 13.
#pragma once
...
enum class EMemoryFragmentTrigger : uint8
{
    ...
    // С 2026-09-19 (23_Journey_Order §23.7) биом смотрят и фрагменты на
    // LowLocalDistortion/ShrineRestored выше: ТИХОЕ МЕСТО и ПЕРВАЯ ВАРКА --
    // Лесостепь, ПОДНОШЕНИЕ -- Пойма, БРОД -- Болото ночью.
    ...
    // биома неочевиден; приближено через IsLegendaryRegionRestored("Баба-Яга")
...
struct PROJECTHERBALIST_API FMemoryFragmentDefinition : public FTableRowBase
{
    ...
    // Насколько подлинный сбор поднимает якорь GlobalPerceptionClarity.
    // 0.1 (23_Journey_Order §23.6, 2026-09-19): девять фрагментов пути +
    // один на Буяне = ровно 1.0; ХЛЕБ-СОЛЬ — сайдовый, 0. До этого было
    // 0.05, и полный якорь был достижим только до 0.5.
    UPROPERTY() float ClarityGain = 0.1f;
