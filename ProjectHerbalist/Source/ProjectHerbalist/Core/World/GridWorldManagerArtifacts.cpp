...
// §21.3-21.4, 2026-09-01). Единственный реально прошитый игровой эффект —
// Дубинка, расчищающая клетку под базу (см. RegisterBase в
// GridWorldManagerBases.cpp) — остальные семь артефактов отслеживаются
// честно (приобретение + честный/обманный путь + структурный аккумулятор
// прогрева), но их конкретный игровой эффект НЕ выдуман: §21.4 сама
// перечисляет только "направление" (не финальный список), §21.5 прямо
// называет пороги прогрева открытым вопросом следующего прохода.
