...
public:
    ...
    // Регион Легендарного очищен (решение пользователя 2026-09-19): Благие
    // проявлены и в покое, а награды за них -- честный артефакт, перья,
    // прогрев, НЕ ПОХВАЛИЛА -- по-прежнему за очищение: прежний гейт карточки
    // (Морок биома ниже MorokThreshold или капище рядом). Злые и Берегиня --
    // то же, что проявление.
    ...
    // Списать заряд -- после любой варки, где он был активен: обычной
    // (RunSimulationStep) и ритуальной (TryAdvanceRitual, идёт мимо очереди
    // команд; с 2026-09-19 -- тоже списывает).
    void SpendBifurcationCharm();
...
private:
    ...
    // double, как сами часы (аудит 2026-09-26, Б8): float терял точность на
    // долгом прохождении, и догон спящих чанков после загрузки уплывал.
    TMap<FIntPoint, double> ChunkLastSimulatedGameTime;
