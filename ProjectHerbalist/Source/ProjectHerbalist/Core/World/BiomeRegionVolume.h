...
public:
    ...
    // размером клетки, а он теперь выводится из ландшафта. Значения на клетку
    // 10 м совпадали со значениями на 100 м²; редирект старых имён снят
    // 2026-09-13, когда ни в одном ассете их не осталось. Число на конкретную
    // клетку -- AGridWorldManager::RollResourceCount.
    ...
    // Программная альтернатива ручной расстановке точек сплайна во
    // вьюпорте (2026-09-06, "сделай тестовую карту в движке" — коммандлет
    // собирает уровень целиком без интерактивного редактора). Заменяет
    // ВСЕ текущие точки сплайна замкнутым многоугольником в МИРОВЫХ
    // координатах — то же координатное пространство, что уже
    // IsPointInside/UpdateCachedPoints читают через
    // ESplineCoordinateSpace::World. Авторская расстановка через редактор
    // (двигать точки во вьюпорте) остаётся основным путём для настоящего
    // контента — это только хук для процедурной/коммандлет-сборки.
    UFUNCTION(BlueprintCallable, Category = "Biome")
    void SetSplinePointsWorld(const TArray<FVector>& WorldPoints);
