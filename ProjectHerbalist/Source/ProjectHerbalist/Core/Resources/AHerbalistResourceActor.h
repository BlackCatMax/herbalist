...
private:
    ...
    // Заспавнен ли актор самой сеткой (2026-09-03, стриминг). ОТДЕЛЬНЫЙ
    // флаг, а не bInitializedFromCode выше: тот выставляется ещё и
    // дозаправкой из реестра, через которую проходит PCG-актор — по нему
    // сетка приняла бы чужой актор за свой и уничтожила при деактивации.
