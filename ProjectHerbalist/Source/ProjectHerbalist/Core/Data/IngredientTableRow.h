...
struct PROJECTHERBALIST_API FIngredientTableRow : public FTableRowBase
{
    ...
    // ни в одной. Степь (8), Лесостепь (9) и Речная пойма (11) проверены и
    // размечены (см. ingredient_gathering_and_garden_flags.json) -- ни
    // одного мотива для bIronAverse/bDelicate ни в одном из трёх не
    // нашлось, только GardenNiche. Речная пойма закрепила границу:
    // деревья/кустарники сознательно БЕЗ ниши (Ива/Ольха/Калина/Смородина)
    // -- все 5 ниш описаны как малые постройки под траву/гриб, ни одного
    // прецедента с многолетним кустом/деревом нет.
    // Оставшиеся 4 биома (Смешанный лес/Тайга/Широколиственный лес/
    // Болото) -- впереди.
