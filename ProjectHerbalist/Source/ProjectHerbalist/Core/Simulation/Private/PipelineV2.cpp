...
namespace Simulation
{
                                        ...
                                        EMoonPhase MoonPhase = EMoonPhase::NewMoon)
    {
        ...
        if (BiomeCtx)
        {
            // ОБА поля контекста с 2026-09-07 -- знаковые отклонения от
            // природы биома, а не абсолютные уровни. Варку интересует именно
            // "насколько это место испорчено/чисто" в абсолюте, поэтому обе
            // строки читают AmbientMorok (дефолт биома + отклонение), а не
            // поля напрямую: иначе в покое отклонение равно нулю, и место
            // перестало бы влиять на зелье вовсе -- тихая, никем не
            // заказанная смена баланса варки.
            EffectiveMorok = FMath::Clamp(BiomeCtx->AmbientMorok * BiomeCtx->MorokAffinity, 0.f, 1.f);
            EffectiveZaryana = FMath::Clamp((1.f - BiomeCtx->AmbientMorok) * BiomeCtx->ZaryanaAffinity, 0.f, 1.f);
