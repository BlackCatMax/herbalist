...
public:
    ...
    //
    // Калибровка: тот же порядок величины, что уже ResourceRegrowthTime=420с
    // (7 реальных минут представляют дни-недели отрастания растения,
    // GridWorldManager.h) — обе механики абстрагируют многодневный реальный
    // процесс до "заметного, но не многочасового" ожидания. 480с (8 минут)
    // взято чуть выше этой планки и совпадает с WeatherFrontDurationSeconds
    // (тоже 480с = четверть игровых суток при дефолтном GameDayMinutes) —
    // не совпадение ради совпадения, а тот же самый порядок "четверть
    // игровых суток", уже откалиброванный в проекте под другую механику:
    // реальная сушка на приличном сквозняке в тёплую погоду ощутимо
    // подвяливает большинство трав за половину-целые сутки, "четверть
    // игровых суток" читается как "хорошая тёплая пора дня", тот же
    // компромисс между честностью и играбельностью, что уже принят у
    // ResourceRegrowthTime.
    UPROPERTY(config, EditAnywhere, Category = "Storage|Drying", meta = (ClampMin = "1.0"))
    float DryingDurationSeconds = 480.0f;
