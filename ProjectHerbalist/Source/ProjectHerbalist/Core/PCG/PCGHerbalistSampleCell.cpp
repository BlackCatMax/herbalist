...
bool FPCGHerbalistSampleCellElement::ExecuteInternal(FPCGContext* Context) const
{
    ...
    // Признак «клетки уже есть» -- у менеджера созданы страницы (HasCellPages).
    // Раньше проверялась первая клетка сетки, но её страница бывает выгружена:
    // угол L_TestDev далеко от игрока (этап 8в) или страница-заполнитель после
    // расширения сетки до мест за убранными плитками (ревью 2026-09-13) -- и узел
    // до конца сессии пропускал точки.
    if (!Manager || !Manager->HasCellPages())
    {
