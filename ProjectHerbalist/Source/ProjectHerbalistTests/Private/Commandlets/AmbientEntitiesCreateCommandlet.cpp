...
namespace
{
    ...
    TArray<FAmbientEntityDefinition> BuildAmbientRows()
    {
        ...
        // Листовики (Смешанный лес, земля): осень (IsAutumn, сентябрь–ноябрь;
        // до календаря 2026-09-16 -- конец Лета) -> Nature++.
        // Зарегистрировано ПОСЛЕ Древесных огней/Шишиг -- в отличие от них,
        // осень не требует ночи/сумерек, без этого порядка забирало бы клетку
        // даже ночью всю осень.
        {
            FAmbientEntityDefinition D;
        ...
        // Плескуны (Речная пойма, ВОДА): Nature-доминанта поймы.
        //
        // Было `Purity >= 0.0` с пометкой "тонкий гейт, почти всегда истинно
        // -- декоративный". На оси [0,1] это не порог вовсе, и последствие
        // оказалось не декоративным: Низшие все одного ранга, CanManifest не
        // даёт вытеснить равного, поэтому Плескуны занимали КАЖДУЮ водяную
        // клетку поймы на первом же тике и навсегда закрывали Русалок,
        // Кувшинкиных духов и Омутные огни. До 2026-09-07 этого не было
        // видно: тесты молча гонялись на нулевых дефолтах биомов.
        //
        // Новый гейт взят из карточки компендиума (04_Compendium/Бестиарий/
        // Речная пойма/Плескуны.md): d_manifest = [0.2, 0.2, 0.5, 0.8], то
        // есть Nature -- доминирующая ось существа; Meta-порога карточка не
        // задаёт вовсе. Порог 0.40 против природной Nature поймы 0.275 (после
        // NormalizeSum) -- тот же запас, что у уже принятых Трясинных духов
        // (0.40 против 0.30 у Болота) и Чащобных духов (0.45 против 0.25 у
        // Тайги). Решение пользователя 2026-09-07.
        {
            FAmbientEntityDefinition D;
        ...
        // Шептуны (Тундра, земля): Spirit-доминанта.
        //
        // Было `Stability >= 0.0` -- та же вырожденность и то же последствие,
        // что у Плескунов выше: Шептуны занимали каждую сухую клетку тундры
        // на первом тике и навсегда закрывали Метельников (метель), Ледяных
        // духов (зима) и Снежные огни (ночь).
        //
        // Новый гейт -- из карточки компендиума (04_Compendium/Бестиарий/
        // Тундра/Шептуны.md): d_manifest = [0.1, 0.5, 0.7, 0.4], Spirit --
        // доминирующая ось ("голоса разносятся ветром"); Meta-порога карточка
        // не задаёт. Порог 0.40 против природного Spirit тундры 0.30 (после
        // NormalizeSum). Решение пользователя 2026-09-07.
        {
            FAmbientEntityDefinition D;
            ...
            D.Biome = EBiomeType::Tundra;
            D.bLandOnly = true;
            D.TriggerAxis = EAmbientTriggerAxis::Spirit;
            D.TriggerThreshold = 0.4f;
            D.bTriggerAbove = true;
            Defs.Add(D);
        }
