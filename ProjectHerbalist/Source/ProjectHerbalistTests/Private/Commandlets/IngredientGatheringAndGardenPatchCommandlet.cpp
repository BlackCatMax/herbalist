...
int32 UIngredientGatheringAndGardenPatchCommandlet::Main(const FString& Params)
{
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("IngredientGatheringAndGardenPatch: не удалось разобрать текущий JSON таблицы"));
        return 1;
    }
    ...
    for (const TSharedPtr<FJsonValue>& Value : LiveRows)
    {
        ...
        // Только bIronAverse/bDelicate/GardenNiche -- всё остальное на живом
        // ряду не трогаем.
    ...
    for (const auto& Pair : PatchByName)
    {
        ...
        {
            UE_LOG(LogTemp, Error, TEXT("IngredientGatheringAndGardenPatch: ряд '%s' из патча не найден в живой таблице"), *Pair.Key);
            return 1;
        }
    }
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("IngredientGatheringAndGardenPatch: %s"), *Problem);
    }

    const int32 FinalRowCount = Table->GetRowMap().Num();
    if (FinalRowCount != OriginalRowCount)
    {
        UE_LOG(LogTemp, Error, TEXT("IngredientGatheringAndGardenPatch: ожидалось %d рядов (не добавляем/не удаляем), получилось %d — не сохраняю"),
    ...
    UE_LOG(LogTemp, Display, TEXT("IngredientGatheringAndGardenPatch: %s -- пропатчено %d из %d рядов (патч содержал %d записей)"),
