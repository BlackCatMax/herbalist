...
int32 UCompendiumAuditCommandlet::Main(const FString& Params)
{
    ...
    for (const FCard& Card : Beasts)
    {
        ...
        // были реализованы Основным рангом, а Низшим их называет карточка.
        // Это расхождение РАНГА, а не пропажа: сгенерировать по такому
        // отчёту заготовки значило бы наделать дублей. (Те три с тех пор
        // перенесены в Низший, -run=BestiaryRankMove; проверка остаётся --
        // она ловит класс ошибки, а не три конкретных имени.)
