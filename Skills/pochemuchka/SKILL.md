...
author: Andrew Anashkin (Cowork adaptation of the original Hermes `pochemuchka` skill)
license: MIT
metadata:
  related_skills: [loop, dod-completion-gate]
---

# Pochemuchka — Prompt Completeness Gate (Cowork edition)

## Why this version exists

The gating logic is unchanged. Two things don't carry over: the "Hermes-wide Enforcement" section (installing this skill across Hermes profiles via a `SOUL.md` fallback) has no Cowork equivalent and is dropped; and where the original just said "ask," this edition points at the actual mechanism available here — the `AskUserQuestion` tool — for anything that fits its constraints (1-4 questions, 2-4 options each), falling back to a plain numbered list in text for anything that doesn't.
...
If anything is unresolved: stop before planning or implementation, and ask all currently known questions at once. Prefer `AskUserQuestion` when the questions fit its shape (up to 4 questions, 2-4 mutually exclusive options each, with a recommended option first); use a plain numbered list in text for open-ended or higher-cardinality questions it can't express. Don't ask for anything already available through safe discovery. After asking, wait — don't begin partial implementation in parallel.
...
Don't pad the list with ceremonial questions — completeness is the goal, not question count.
...
7. Re-asking for information already provided or retrieved.

## Verification Checklist

- [ ] Safe discovery was completed where useful.
