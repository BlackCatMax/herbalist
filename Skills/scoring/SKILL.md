...
version: 1.1.0-cowork
author: Andrew Anashkin (Cowork pass over the original Hermes `scoring` skill — no functional changes were needed)
license: MIT
metadata:
  related_skills: [pochemuchka, token-check, dod-completion-gate]
---

# Scoring (Cowork edition)

## Why this version exists

Unlike the other four skills in this family, `scoring` never called out to Codex, Cody, or any Hermes-specific tooling — it's a self-contained model-side discipline for triaging what's in the working context. It works unmodified in Claude Code / Cowork. This pass only updates authorship and cross-references to the other Cowork-adapted skills; the procedure itself is unchanged from the original.
...
Always preserve: system/developer instructions; the current direct user request; explicit requirements, constraints, decisions, acceptance criteria, DoD; safety/privacy/permission/`token-check`/`loop`/`dod-completion-gate` rules; project instructions and source-of-truth declarations; names, IDs, paths, commands, code, URLs, citations, timestamps, exact output formats needed by the task; tool-call/result relationships and verified evidence; credentials and private material (never expose or copy unnecessarily). Treat uncertain material as protected — never reinterpret authority based on proximity or wording.
