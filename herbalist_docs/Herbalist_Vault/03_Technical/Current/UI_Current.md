...
- Три слота ингредиентов + слот воды + слот-витрина результата
  ...
  см. `ROADMAP.md`).
- Превью Coherence нет: `ComputeIntentCoherence` — `static` внутри
  `PipelineV2.cpp`, считается только при самой варке.
- «Смешать» → `AGridWorldManager::QueueCauldronBrew`: команду собирает
  менеджер, модификаторы варки те же, что у применения в клетку
  (`ResolveBrewModifiers` — заряд Камня-оберега, фаза луны, BrewBoost,
  межбиомность, тиражный оберег). Ингредиенты помечены
  `bIngredientsAlreadyWithdrawn` — они ушли из сумки ещё при переносе в слоты,
  Pipeline их второй раз не списывает (2026-09-14).
- Результат варки — сразу в сумке; слот результата — витрина, наполняется из
  `AGridWorldManager::OnBrewCompleted` настоящим предметом. Занятая витрина
  варку не блокирует. Статус называет исход: зелье, зола (нет воды),
  кипячёная вода (нет трав) (2026-09-14).
- Полная сумка (`MaxSlots` строк заняты) — варка не начинается: результату
  нужен свободный слот, иначе `AddItem` отказал бы, а травы уже в котле.
...
### Размеры окон
- Окна котла, сумки, их слоты и подсказка растут под текст: в `NativeConstruct`
  `HerbalistUI::LetSizeBoxesGrowWithContent` (`UI/HerbalistWidgetSizing.h`)
  превращает фиксированный размер `SizeBox` из макета WBP в минимальный.
  Ассеты не меняются (2026-09-14).
