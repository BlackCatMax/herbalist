...
- `FindRealIndex` резолвит слот по стабильному `CreationTime`, не по
  ...
  — раньше мог найти чужой слот). Сначала — свой номер строки: одна стопка,
  разошедшаяся по станциям, даёт несколько строк с одним `CreationTime`
  (2026-09-14).
- Тултип — `UItemTooltipWidget` при наведении, показывает человекочитаемое
  имя травы (`GetItemDisplayName`), не сырой `IngredientID`.
- Искажённая копия предмета — `ResolvePerceivedItem`: сумка игрока берёт кэш
  восприятия, если под номером строки тот же предмет; предметы хранилищ и
  станций искажаются на месте той же `ComputePerceivedInventory` (восприятие
  менеджера считает только сумку). До 2026-09-14 слот любого инвентаря брал
  предмет сумки с тем же номером.
- В строке типа тултипа — состояние процессов станций с настоящего предмета
  (`GetItemProcessStatus`): «сохнет, осталось N мин», «высушено», «сушка не
  закончена, осталось N — в сушилку», так же отстой и выпаривание (2026-09-14).
