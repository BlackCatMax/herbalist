...
**Tech:** Метод `UBiomeGraphSubsystem::RecordFootprint(BiomeID, MorokImpact, ZaryanaImpact, AxisDelta, Weight)`. Пайплайн (`PipelineV2.cpp`) производит `Delta.Footprints` как чистые данные; применяет их `AGridWorldManager::RunSimulationStep` (в `Core/World/GridWorldManagerTick.cpp`) — не `ApplyAlchemyResult`, такой функции с обработкой следов нет. Обновляет `FBiomeMemory` узла: добавляет в `MorokHistory`, `ZaryanaHistory`, увеличивает `Instability`, накапливает `AxisDrift`.
