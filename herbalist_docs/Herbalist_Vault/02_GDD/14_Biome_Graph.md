...
> Техническая сторона главы — [[14_Biome_Graph_Tech]] (разделы с теми же номерами или названиями);
...
Формально биом описывается структурой `FBiomeGraphNode` (см. [[14_Biome_Graph_Tech]]).

## 14.2 Мир как граф состояний

Мир представлен как ориентированный взвешенный граф:
- **Узлы** — биомы с параметрами (`MorokAffinity`, `ZaryanaAffinity`, `Stability`) и
runtime-полями (`MorokField`, `ZaryanaField`, `Memory`).
- **Рёбра** — направленные связи с коэффициентами утечки (`MorokLeak`,
`ZaryanaFlow`).

**Уточнение 2026-09-07:** `MorokField`/`ZaryanaField` — не абсолютный уровень,
а **знаковое отклонение** [-1, 1] текущего Distortion/Stability биома от его
природного (дефолтного) значения. Ноль поля = «биом в своей природе», не
«искажения нет» в абсолютном смысле. Абсолютный уровень (для порогов
Легендарных, вклада места в варку) — `UBiomeGraphSubsystem::GetAmbientMorok(BiomeID)` =
`clamp(дефолт биома + поле, 0, 1)`. Заряна больше не зеркало Морока (не
`1 - Distortion`) — отдельная величина, отклонение Stability. Полный вывод —
`MATH_REFERENCE.md §3, §6.1–§6.2`.
...
Алгоритм:
1. **Grid → Graph:** поля узлов агрегируются из состояния клеток
(суммы по биому из сводок чанков, `GetBiomeFieldSums`) как отклонение от дефолта биома, не как абсолютное значение.
2. **Propagation:** с 2026-09-07 — настоящая **консервативная диффузия**: поток
вычитается из источника и прибавляется к приёмнику, сумма поля по всему графу
сохраняется (до этого поток копировался в обе стороны, что создавало Морок
из ничего; тест `Herbalist.BiomeGraph.PropagateWavesConservesTotalMorokAcrossAllNodes`).
3. **Graph → Grid:** поля узлов влияют на `TargetState` клеток (мягкая
интерполяция, «дырявое ведро» к дефолту биома + поле), пропускается целиком
для клеток в бистабильном полюсе (`Cell.Memory.bDegrading`).
...
|---------|-------------------|
| [[GridWorldManager]] | Источник истины (состояние клеток). Предоставляет `GetBiomeFieldSums()` и `GetBiomeCenters()` из сводок чанков. |
...
> **Техническая сторона:** [[14_Biome_Graph_Tech]]
