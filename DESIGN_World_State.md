...
1. **Модель хранения** — выбрать из четырёх вариантов (§3). Рекомендация:
   A+B ядро, C визуал.
2. ✅ ВЫПОЛНЕНО 2026-08-23 — **Звено 3**, в форме §15 (двухступенчатый фильтр:
   `AllowedBiomes` решает «кто», близость `Cell.State` к `Row.BaseState` — «сколько»),
   а не радикальной §8. Открывает 8 (сезонный множитель — тот же формуле одним
   сомножителем, ещё не сделано: сезонов пока нет).
...
**✅ ВЫПОЛНЕНО 2026-08-23** (`GridWorldManagerEntities.cpp`): `GetEntityManifestationPriority`
(Легендарный > Основной > Низший, по рангу бестиария) и `CanManifest` — более
приоритетный хозяин вытесняет менее приоритетного при совпадении условий, а не
побеждает тот, чей блок кода выполнился последним по порядку в тике. Гистерезис —
`PassesHysteresisThreshold` (триггер Шмитта, общий запас `EntityManifestationHysteresis`)
на всех трёх порогах: Corruption Гнильников, HistoryPurity Берегини, Respect
Полевика (у него — на обоих полюсах, благословение и порча, по знаку Respect).
Без выделенного автотеста: проверка требует тикающего `AGridWorldManager` в мире,
несоразмерно объёму правки; корректность проверена вручную построчно и полным
прогоном регрессии (8/8).
...
**Статус (2026-09-04) — ботаническая сверка `AllowedBiomes`.** Строка 376 выше
("не трогаем") оставалась верна как ПРИНЦИП — биом решает «кто», состояние
решает «сколько» — но сами данные под ним были не тронуты со старта: у всех 76
карточек `AllowedBiomes` был ровно один биом, тот же, что подсказывало имя
файла (`bol_*`→Болото, `tai_*`→Тайга и т.д.), без сверки с реальными ареалами.
Статистика по факту: одно Болото в одиночку давало 57% всего разнообразия
«типов зелий» игры (эпитет+ось) — не ботаника, а перенос "что было под рукой".

Фоновый агент прошёл все 76 карточек по `DisplayName`/`Description` (реальные
русские названия растений/грибов) и сверил природный ареал с ботаническими
знаниями: 70 из 76 карточек получили расширенный, ботанически обоснованный
список биомов (например, Багульник болотный растёт не только на верховых
болотах, но и в заболоченной тайге и мохово-кустарничковой тундре — те же
условия, что у сфагнума и клюквы; Аконит — не эксклюзив глухой тайги, виды
рода обычны и в тенистых смешанных лесах). 6 карточек сознательно оставлены
однобиомными как настоящие ботанические эндемики/специалисты проекта —
Сфагновые мхи, Плакун-трава, Стрелолист, Катран (Степной хрен), Тюльпан
степной, Полярный мак — тот же принцип "пробелы допустимы и честны", что уже
применялся к нишам сада. Приоритет №2 (баланс осей Body/Взвар и Mind/Дурман,
слишком редких из-за прежней изоляции по одному биому) учитывался только там,
где ботаника САМА оставляла выбор между несколькими подходящими биомами
разных ярусов риска (`DT_BiomeDefaults.EntityActivityBase`) — не изобретая
смещений, которых нет в реальном ареале. Подробности и разбивка по видам —
`CHANGELOG.md`, запись 2026-09-04; техническая правка живого
`DT_IngredientClass` — точечно через `IngredientBiomeRangePatchCommandlet`
(тот же приём `FindRow`, что и у `IngredientGatheringAndGardenPatchCommandlet`,
без полного `GetTableAsJSON()`/`CreateTableFromJSONString()`-роундтрипа).
Следующий, ещё не начатый шаг — рецептурный гейт между ярусами биомов
(см. `ROADMAP.md`, пункт про биомный дисбаланс): само `AllowedBiomes` теперь
честнее, но механики, которая делала бы редкие/дальние биомы игроку ценнее
именно как источник уникальных рецептов, пока нет.
...
**✅ ЗАКРЫТО 2026-08-30** ("закрываем архитектурный долг"). Оба пункта решены
переименованием/удалением, не построением графа зависимостей: ни у одной из
шести команд (Query/Transfer/Apply/Harvest/Wait) нет и не было причины
зависеть от другой в пределах одного тика — строить граф ради этого значило
бы решать несуществующую задачу. `Talk`/`FTalkCommand` удалены целиком, не
доведены до реализации — не просто пустой каркас, а мёртвый код (ни один
игровой путь никогда не создавал `FTalkCommand`) вдобавок к тому, что
`17_Hero_And_Community.md` прямо отвергает диалог как механику ("вместо
диалоговой благодарности... тихая ответная плата"). `FCommandGraph`
переименован в `FCommandBatch` — честное имя для того, чем структура и была
(плоский `TArray<FCommandEntry>` на один тик). Заодно удалён мёртвый
`ExecutionOrder` (объявлен, нигде не читался и не сортировался по нему —
реальный порядок исполнения всегда был порядком добавления в `PipelineV2.cpp::
ExecutePipeline`, обычным `for`). Q/T/D/S остаются на месте без изменений.
Регрессия не нужна была отдельно — существующие `Herbalist.PipelineV2.*`/
`Herbalist.MoonPhase.*`/`Herbalist.Resilience.*` уже гоняют
`FCommandBatch`/`ExecutePipeline` и подтвердили, что переименование ничего
не сломало. 86/86 всего зелёные.
...
> [!warning] Superseded (2026-09-06)
> §24-32 ниже (до горизонтальной черты после §32) — дизайн-предложение от
> 2026-08-23/24, само себя честно помечающее статусом "код не начат" (см.
> §32). Реальная реализация бестиария/капищ/оберегов пошла ДРУГИМ путём:
> обобщённый `FInfluenceSource` не заведён вовсе — вместо одного общего типа
> с полем `Kind` в проекте по-прежнему отдельные конкретные структуры
> (`FEntityLandmark` для хозяев места, `FShrine` для капищ, тиражные обереги
> и курганы — каждый свой набор полей на `AGridWorldManager`), а Низший ранг
> (Гнильники и т.д.) вообще не хранит персистентный "источник" — его
> присутствие каждый тик заново считается порогом по `Cell.State`
> (`AmbientEntityTypes.h`/`UpdateEntityManifestations`), не отдельным
> объектом с позицией/радиусом/силой. Четыре механизма "живости" (§28-31)
> предполагали `FInfluenceSource` как фундамент и тоже не реализованы —
> `DistortionVelocity`/`TimeOfLastDistortionChange` (§30) по-прежнему
> объявлены и не читаются нигде в коде. Оставлено ниже как исторический
> дизайн-документ, не текущая архитектура — см. `03_Technical/Current/Core_Current.md`
> за тем, что реально построено.
...
### 26. Обновлённый порядок (заменяет часть Фазы A из тогдашней `ROADMAP.md`,
см. `CHANGELOG.md` 2026-08-23/24 за фактической историей Фазы A)
...
Дальше план не менялся: Фаза B (S0, Distance, Травник), Фаза C
...
## Часть VIII. Бистабильная релаксация — точка невозврата (обсуждение 2026-08-24)

> ✅ **Перенесено в канон 2026-08-30** — `02_GDD/12_Biome_Change.md §12.10`
> (бистабильность/гистерезис) и `02_GDD/05_Systems.md` «Передозировка»
> (§40). Раздел ниже сохранён как рабочая история решения (в т.ч.
> отклонённая альтернатива §39), не дублируется здесь дословно в каноне.

### 37. Вопрос из сессии

До сих пор `RegenerateCellParameters` — один бассейн притяжения: клетка
всегда тянется к `TargetState` биома, сколько бы Corruption на ней ни было.
Вопрос, поднятый в сессии: должна ли быть граница, ниже которой пассивное
самовосстановление перестаёт работать и начинается самоусиливающаяся
деградация, пока игрок не продавит клетку обратно активным вмешательством?
И — отдельно, но рядом — нужна ли геймплейная причина **держать** биом
заражённым (яды), и асимметричны ли скорости порчи/очищения.

### 38. Решение — общий случай того, что уже делали Гнильники

Гнильники (`UpdateEntityManifestations`, до этой правки) уже реализовывали
ровно эту идею, но узко: только для Болота, только сдвигая `TargetState.
Corruption`/`Purity` в сторону порчи, без реального второго устойчивого
состояния и без выхода. Обобщение — та же идея на любую клетку любого биома,
с гистерезисом на обе стороны:

```
Corruption клетки > 0.85 (центр 0.75 + запас 0.10) → TargetState клетки
    сам сдвигается на испорченный полюс (Corruption→1, Purity→0,
    Distortion→1, Stability→0) — пассивная релаксация теперь усугубляет,
    а не лечит.

Corruption < 0.65 (центр − запас) → TargetState возвращается к здоровому
    умолчанию биома (или воды).

Между 0.65 и 0.85 — зона гистерезиса: режим не меняется сам по себе,
только по факту пересечения одной из границ (тот же Триггер Шмитта,
что уже есть у проявления сущностей, `PassesHysteresisThreshold`, теперь
общий — `HerbalistCoreMath.h`).
```

Единственный выход из деградации — не ожидание, а прямое действие игрока
(например, `ApplyPotionToCell` с очищающим зельем), продавливающее
`State.Meta.Corruption` ниже 0.65 достаточно резко, чтобы гистерезис
успел это заметить.

**Почему центр 0.75, а не 0.5 или 0.9:** здоровый Corruption Болота по
`DT_BiomeDefaults.json` уже 0.70 — специально внутри полосы гистерезиса
(0.65–0.85). Самый тёмный из «нормальных» биомов и должен естественно
сидеть ближе всех к точке невозврата: небольшое дополнительное
вмешательство (неудачная варка, интенсивный сбор) может качнуть его в
любую сторону, тогда как у Тайги (Corruption 0.18) огромный запас прочности
до той же границы. Это не подгонка числа под биом, а следствие уже
принятого решения о том, что Болото — не «плохой» биом, а балансирующий
на грани.

**Капище и бистабильность совместимы без единой правки в самом капище:**
эффект 1 (§15.5) сравнивает `Distance(TargetState, S0)` с `Distance(State,
S0)` — если клетка деградирует, `TargetState` теперь далеко от S0, и
капище само распознаёт шаг как «отдаление», замедляя его тем же
механизмом, что уже работал для обычной порчи. Капище не останавливает
деградацию полностью (v1 не про это), но сопротивляется ей — «повышают
устойчивость... к дурным переменам» из канона выполняется буквально.

### 39. Стимул держать биом заражённым — проверено против канона, отклонено

Второй вопрос сессии (яды как причина не чинить биом, «отыгрыш тёмной
стороны») — до реализации проверено против уже написанного канона
(`08_Content.md`, компендиум трав, `03_Narrative.md`) и не подтвердилось:

- **В компендиуме яд — не оружие, а передозировка лекарства.** Ландыш: «сок
  его — сильное сердечное зелье, но и яд лютый». Тот же паттерн у Полярного
  мака (обезболивающее/эвтаназия, смертельно при передозировке) и Чистотела.
  Ни одна карточка не описывает яд как отдельное «злое» растение против
  «доброго» — это один и тот же стебель, разница в дозе, не в природе.
- **«Тёмного пути» в нарративе нет.** История строго однонаправленная: герой
  снижает Distortion/Corruption, идёт к Буяну — состоянию, прямо
  противоположному порче. Введение «выгодно поддерживать порчу» было бы
  новым каноном, не находкой в старом.
- **Некуда было бы применить.** В проекте нет ни экономики, ни боёвки, ни
  квестовой системы — «преимущества ядов» физически негде реализовать
  механически, даже вне вопроса канона.

**Решение (2026-08-24): пойти по лорной ветке вместо этой.** За порчу
по-прежнему отвечает Морок — не переопределяется. Вместо «держи биом
грязным» — «сила лекарства и его яд одна ось» (см. §40): стимул не про
биом, а про дозу зелья при применении.

### 40. Передозировка зелий — реализовано

`HerbalistCore::Math::ApplyOverdosePenalty` (`HerbalistCoreMath.h`), вызывается
в `ProcessApplyCommand` (`PipelineV2.cpp`) при применении зелья на клетку —
не при крафте, передозировка это событие применения, не варки. Выше порога
`Potency` (0.75 по умолчанию, `UHerbalistSettings::PotionOverdoseThreshold`)
зелье вместо усиления пользы линейно до бесконечности начинает толкать
`Stability`/`Purity` вниз и `Distortion`/`Corruption` вверх — тем же
четырёхосевым «испорченным» профилем, что уже определяет деградацию клетки
в §38 (единый язык «вреда» по всему проекту, не второй отдельный).

Побочный эффект, ожидаемый и правильный: передозировка на клетке капища
(§15.5, эффект-подношение через `Delta.WorldChanges`) реально может его
осквернить — не потому что порча зелья запланирована отдельно, а потому что
формула подношения (`Purity − Corruption`) честно читает то же испорченное
состояние. Один источник правды, не два пересекающихся правила.

Осознанно не сделано: «уязвимость цели» (хрупкая клетка страдает от
передозировки сильнее устойчивой) — дешёвое расширение через
`TargetCell->State.Meta.Stability`, но не в этом проходе, чтобы не
разрастить v1 не по запросу.

---

## Часть IX. Заряна: фрагменты памяти и Буян (2026-08-24)

> ✅ **Перенесено в канон 2026-08-30** — `02_GDD/17_Hero_And_Community.md
> §17.4-17.5`, включая экран "читать воспоминание" (2026-08-30). Раздел
> ниже сохранён как рабочая история адаптации пользовательского прототипа
> (§41), не дублируется дословно в каноне.

### 41. Прототип пользователя → адаптация на существующую архитектуру

Пользователь дал развёрнутый прототип, описывающий `EventOutbox`, `CommandBus`,
`AssetCatalog`, `HerbalistRuleSet` — инфраструктуры этого класса в проекте нет
(явно оговорено: «код может быть не верным»). Реализовано на уже существующих
каналах:

| Прототип | Что в проекте |
|---|---|
| `EventOutbox`/`CommandBus` | внепайплайновый тик (`UpdateMemoryFragments`, тот же принцип, что `UpdateShrines`/`UpdateEntityManifestations`) + событийный крючок в `RunSimulationStep` (та же точка, где уже читается Coherence для Травника) |
| `FMemoryFragmentDefinition` (DataAsset) | хардкод, `Core/Zaryana/MemoryFragmentDefinitions.h` — 3 определения, вертикальный срез (тот же масштаб решения, что бестиарий 4/62, капища 3/4 эффекта) |
| `MemoryFragmentSpawner`/`MemoryFragmentCollector` | методы `AGridWorldManager` напрямую — нет причин заводить отдельные UObject-сервисы под три спрятанных условия |
| Мировое проявление | `AMemoryFragmentActor` — тот же паттерн, что `AHerbalistResourceActor` (сфера взаимодействия) и `Interact()` (уже используется для `AAlchemyTableActor`/`AStorageContainer`) |
| `GlobalPerceptionClarity` | поле на `AGridWorldManager`, гасит `NoiseScale` в `PerceiveRealState` (новый опциональный параметр `Clarity`, default 0 — старое поведение везде, где не касается) |

### 42. Честное противоречие с 06_Progression — не скрыто, разрешено пользователем

`06_Progression.md`: «в игре отсутствуют уровни, числовые характеристики и
прямые улучшения». `GlobalPerceptionClarity` — это ровно числовое улучшение
восприятия, то же по форме, что было отвергнуто для пункта 8 Фазы C (там —
сравнение записей Травника вручную, никакого снижения шума бэкендом). Разница,
которая делает исключение осмысленным, а не подтасовкой: фрагментов **мало**
(3 в v1, не бесконечный фарм) и каждый — сюжетный бит, полученный один раз, не
награда за повтор действия. Ближе к «герою передали знание» (обряд знахарки на
смертном одре, уже в каноне), чем к скрытому скилл-триггеру. Это решение
пользователя, данное явно вместе со спекой фрагментов, не моя тихая трактовка.

### 43. Ложные фрагменты — единственный сигнал различия зашит в контекст, не в текст

По спеке: при высоком глобальном Distortion подлинный триггер всё же может
дать ложную версию (`SpawnMemoryFragmentAt`, вероятностный ролл на среднем
Distortion не-водных клеток — отдельного агрегата "глобальный Морок" в
проекте нет, заводить его ради одной проверки раз в `MemoryFragmentStateCheckInterval`
секунд не стоило). Сбор ложного фрагмента **не** блокирует будущий подлинный
спавн того же ID (`CollectedFragmentIDs` не трогается) и слегка **понижает**
уже набранную `Clarity`, а не даёт её — риск идти собирать фрагмент в
нестабильном месте настоящий, не декоративный.

### 44. Что осталось на уровне кода, но не контента

- **Локация Буяна** (живая/мёртвая вода) — флаг `bBuyanReached` готов, сама
  локация не существует: level design, не C++, тот же класс ограничения, что
  WBP-блюпринты виджетов.
- **✅ Экран "читать воспоминание" ВЫПОЛНЕН 2026-08-30** ("Прогрессия/Заряна",
  тот же C++-без-WBP приём, что уже дал `JournalLogWidget`). `UI/
  MemoryRevealWidget.h` — экранный попап (`WidgetTree` целиком в коде),
  показывает `Def->TrueText`/`FalseText` в момент сбора вместо одного
  `UE_LOG`; `CollectMemoryFragment` (`GridWorldManagerZaryana.cpp`) вызывает
  его через новый `AHerbalistPlayerController::ShowMemoryRevealText`, и тем
  же вызовом добавляет запись в Травник (`EJournalEntryType::MemoryFragment`,
  `JournalTypes.h`) — воспоминание не теряется после попапа, перечитывается
  позже. `JournalLogWidget` вдобавок показывает текущую `GlobalPerceptionClarity`
  живьём (не кэшированным снимком) рядом с заголовком. `CheckBuyanCondition`
  тем же попапом объявляет достижение Буяна (без контента самой локации —
  этот пункт списка выше остаётся открытым). `ShowZaryanaStatus` (Exec) не
  тронут — остаётся отладочной консольной командой, не заменяется экраном.
- **Более одного капища одновременно в условии Буяна** — код уже требует
  прохода по ВСЕМ `Shrines`, не жёстко зашитому числу, так что готово под
  будущие мастерские без правок, как и просил пользователь («пока заложим
  под будущие, число не решено»).
